...
interface ObjectConstructor {
     ...
     */
    // properties为指定key指定对象描述符
    // 字面量properties的this类型为any
    create(
        o: object | null,
 ...
 */
// 提取出函数类型的this的参数类型，如果没有则返回unknown
// (this: unknown, ...args: any[]) => any为一个普通函数，this为unknown类型
type ThisParameterType<T> = T extends (this: unknown, ...args: any[]) => any
    // 如果函数符合(this: unknown, ...args: any[]) => any，也就是this为unknown类型，则返回unknown
    ? unknown
    // 如果函数不符合，再往下判断
    // 
    : T extends (this: infer U, ...args: any[]) => any
    // 如果函数的this指向U类型，则返回U类型，也就是其真实的类型，这里的infer仅仅作为推导
    ? U
    // 否则返回unknown
    : unknown;
...
type OmitThisParameter<T> = unknown extends ThisParameterType<T>
    // 如果unknown extends ThisParameterType<T>成立，说明该函数的this类型为unknown
    // 直接返回该类型即可
    ? T
    // 否则，说明其有this不为unknown或者没有this
    // 再把结果的R类似拿出，和参数的A类型取出，组成没有this的新函数返回
...
interface CallableFunction extends Function {
     ...
     */
    // 调用具有指定对象的函数作为this值，并将指定数组的元素作为参数。
    // this的类型为(this: T) => R
    // 返回值为R函数
     ...
     */
    //ThisParameterType 拿到T的this的类型
    //OmitThisParameter 拿到T除了this外其他参数的类型
...
interface Array<T> {
     ...
     * @param thisArg An object to which the this keyword can refer in the callbackfn function. If thisArg is omitted, undefined is used as the this value.
     */
    //map的返回值也是一个数组，所以用泛型U来表示新数组子项的类型
    map<U>(
